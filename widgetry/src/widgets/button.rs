...
use crate::{
    ...
    GeomBatch, GfxCtx, Image, MultiKey, Outcome, OutlineStyle, RewriteColor, ScreenDims, ScreenPt,
    ScreenRectangle, Text, TextSpan, Widget, WidgetImpl, WidgetOutput,
};
...
impl<'b, 'a: 'b, 'c> ButtonBuilder<'a, 'c> {
    ...
    pub fn label_underlined_text(mut self, text: &'a str) -> Self {
        ...
        label.styled_text = Some(Text::from(TextSpan::new(text).underlined()));
        self.default_style.label = Some(label);
        self
    }
    ...
    fn batch(&self, ctx: &EventCtx, for_state: ControlState) -> GeomBatch {
        ...
        let label_batch = state_style
            ...
            .and_then(|label| {
                ...
                let mut line = TextSpan::new(text).fg(color);
