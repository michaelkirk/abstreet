            ...
            Style::Area { width } => {
                ...
                batch.push(
                    if self.mouse_on_slider {
                        ...
                        // we don't want to use `ctx.style.btn_solid.bg` because
                        // it achieves it's "dulling" with opacity, which causes the biking
                        // slider to "peak through" and looks weird.
