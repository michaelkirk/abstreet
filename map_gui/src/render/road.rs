...
use widgetry::{Drawable, GeomBatch, GfxCtx, Prerender, Text, TextSpan};
    ...
    pub fn render<P: AsRef<Prerender>>(&self, prerender: &P, app: &dyn AppLike) -> GeomBatch {
        ...
        if !r.is_light_rail() {
            ...
            if r.center_pts.length() >= Distance::meters(30.0) && name != "???" {
                ...
                if false {
                    // TODO Not ready yet
                    batch.append(TextSpan::new(name).fg(fg).render_curvey(
                        prerender,
                        &r.center_pts,
                        0.1,
                    ));
                } else {
                    let txt = Text::from(TextSpan::new(name).fg(fg)).bg(bg);
