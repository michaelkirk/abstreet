...
use widgetry::{
    Drawable, EventCtx, GeomBatch, GfxCtx, Key, LinePlot, Outcome, Panel, PlotOptions, Series,
    TextExt, TextSpan, Toggle, Widget,
};
...
fn make_table<I: Into<String>>(ctx: &EventCtx, rows: Vec<(I, String)>) -> Vec<Widget> {
    rows.into_iter()
        .map(|(k, v)| {
            Widget::row(vec![
                TextSpan::new(k).secondary().into_widget(ctx),
                // TODO not quite...
