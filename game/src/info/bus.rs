...
use widgetry::{Color, EventCtx, Key, Text, TextExt, TextSpan, Widget};
...
pub fn stop(ctx: &mut EventCtx, app: &App, details: &mut Details, id: BusStopID) -> Vec<Widget> {
    ...
    rows.push(Widget::row(vec![
        "Bus stop".span().small_heading().into_widget(ctx),
        header_btns(ctx),
    ]));
    rows.push(TextSpan::new(&bs.name).into_widget(ctx));
    ...
    for r in app.primary.map.get_routes_serving_stop(id) {
        ...
        if let Some((t, _)) = arrivals.last() {
            ...
            txt.add_line(
                TextSpan::new(format!("  Last bus arrived {} ago", sim.time() - *t)).secondary(),
            );
        } else {
            txt.add_line("  No arrivals yet".span().secondary());
        }
...
pub fn route(ctx: &mut EventCtx, app: &App, details: &mut Details, id: BusRouteID) -> Vec<Widget> {
    ...
    rows.push(Widget::row(vec![
        TextSpan::new(format!("Route {}", route.short_name))
            .small_heading()
            .into_widget(ctx),
        header_btns(ctx),
    ]));
