...
use sim::AgentType;
use widgetry::{
    Color, DrawWithTooltips, EventCtx, FanChart, GeomBatch, PlotOptions, ScatterPlot, Series, Text,
    TextSpan, Toggle, Widget,
};
...
pub fn traffic_signal(
    ...
    {
        let mut txt = Text::new();
        txt.add_line(TextSpan::new(format!("{} stages", signal.stages.len())).small_heading());
    ...
    for (idx, stage) in signal.stages.iter().enumerate() {
        rows.push(
            match stage.stage_type {
                StageType::Fixed(d) => TextSpan::new(format!("Stage {}: {}", idx + 1, d)),
                StageType::Variable(min, delay, additional) => TextSpan::new(format!(
                    "Stage {}: {}, {}, {} (variable)",
                    idx + 1,
                    min,
                    delay,
...
fn header(
    ...
    rows.push(Widget::row(vec![
        TextSpan::new(label).small_heading().into_widget(ctx),
        header_btns(ctx),
    ]));
