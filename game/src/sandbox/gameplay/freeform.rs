...
use widgetry::{
    lctrl, Choice, Color, EventCtx, GfxCtx, HorizontalAlignment, Key, Outcome, Panel, SimpleState,
...
impl GameplayState for Freeform {
    ...
    fn recreate_panels(&mut self, ctx: &mut EventCtx, app: &App) {
        let rows = vec![
            ...
            .centered(),
            Text::from_all(vec![
                "Select an intersection and press ".span(),
                Key::Z.txt(ctx),
                " to start traffic nearby".span(),
            ])
            .into_widget(ctx),
        ];
...
impl ChangeScenario {
    pub fn new(ctx: &mut EventCtx, app: &App, current_scenario: &str) -> Box<dyn State<App>> {
        ...
        let mut col = vec![
            Widget::row(vec![
                "Pick your scenario".span().small_heading().into_widget(ctx),
                ctx.style().btn_close_widget(ctx),
            ]),
            "Each scenario determines what people live and travel around this map"
                .span()
                .into_widget(ctx),
        ];
        for (name, label, description) in choices {
            ...
            col.push(
                Widget::row(vec![
                    ...
                    Text::from(TextSpan::new(description).secondary())
                        .wrap_to_pct(ctx, 40)
                        .into_widget(ctx)
                        .align_right(),
                ])
    ...
    fn new(ctx: &mut EventCtx, start: Option<BuildingID>) -> Box<dyn State<App>> {
        let mut spawner = AgentSpawner {
            ...
            panel: Panel::new(Widget::col(vec![
                Widget::row(vec![
                    "New trip".span().small_heading().into_widget(ctx),
                    ctx.style().btn_close_widget(ctx),
                ]),
