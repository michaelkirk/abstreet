...
use widgetry::{
    ...
    Outcome, Panel, PersistentSplit, ScreenDims, Text, TextExt, TextSpan, VerticalAlignment,
    Widget,
};
...
impl TimePanel {
    ...
    pub fn recreate_panel(&mut self, ctx: &mut EventCtx, app: &App) {
        ...
        row.push(
            Widget::custom_row(
                ...
                .into_iter()
                .map(|(s, label)| {
                    let mut txt = Text::from(TextSpan::new(label).small());
    ...
    fn trips_completion_bar(&mut self, ctx: &EventCtx, app: &App) -> Widget {
        ...
        let text_geom = Text::from(
            TextSpan::new(format!("Finished Trips: {}", prettyprint_usize(finished)))
                .fg(text_color),
        )
        .render(ctx)
        .translate(8.0, 0.0);
    ...
    fn create_time_panel(&mut self, ctx: &EventCtx, app: &App) -> Widget {
        ...
        Widget::col(vec![
            Text::from(TextSpan::new(self.time.ampm_tostring()).big_monospaced()).into_widget(ctx),
